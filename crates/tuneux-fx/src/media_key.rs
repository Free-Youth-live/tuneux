...
//! 也能控制 tuneux-fx 播放。各平台实现方式不同：
//!
//! - **Linux**：注册 MPRIS D-Bus 服务（`org.mpris.MediaPlayer2.tuneuxfx`），
...
mod linux {
    ...
    impl MprisPlayer {
        ...
        fn metadata(&self) -> std::collections::HashMap<String, zbus::zvariant::OwnedValue> {
            // 锁中毒安全：即便锁被某线程 panic 污染，也正常取到标题。
        ...
        let handle = match std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let conn = match builder
                    ...
                    .and_then(|b| b.name("org.mpris.MediaPlayer2.tuneuxfx"))
                    .and_then(|b| b.build())
                {
                    Ok(c) => c,
                    Err(_) => {
