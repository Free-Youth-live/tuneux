...
// 本模块与基础版 playlist 同源（随机源已正名 SplitMix64），本产品线尚未接入
// 或不启用的公开接口，临时豁免 dead_code 警告。
#![allow(dead_code)]
...
/// 注：与 `tui/render/spectrum.rs` 的 `rng_next` 同一算法（两处均为 6 行纯函数，
/// 分属不同模块树，未强行合并以免引入跨模块耦合）。
#[derive(Debug, Clone)]
struct SplitMix64 {
    state: u64,
}
...
mod tests {
    ...
    fn peek_next_skips_same_file_cue_fragment() {
        ...
        p.set_current(0);
        // 下一曲（分轨 2）与当前同 path 且带 cue → 不预载
        assert_eq!(p.peek_next(RepeatMode::Off), None);
        // 末曲环绕：下一曲（分轨 1）仍同 path 且带 cue → 不预载
        p.set_current(2);
        assert_eq!(p.peek_next(RepeatMode::List), None);
    }

    /// 预载守卫不影响普通曲目的同路径重复条目（无缝重开是正确行为）。
    #[test]
    fn peek_next_allows_same_path_plain_duplicate() {
        let mut p = Playlist::new();
        p.add(item(None, None, "/same.mp3"));
        p.add(item(None, None, "/same.mp3"));
        p.set_current(0);
        assert_eq!(
            p.peek_next(RepeatMode::Off),
            Some(PathBuf::from("/same.mp3"))
        );
    }

    /// clamp_seek_to_cue：分轨区间钳制的边界。
    #[test]
