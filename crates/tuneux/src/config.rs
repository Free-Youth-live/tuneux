//! # 配置模块
//!
//! 负责 tuneux 运行时配置的加载与持久化。配置以 TOML 格式存储，
//! 文件名固定为 `tuneux.toml`。
//!
//! ## 配置文件位置策略（便携优先）
//!
//! 1. **默认**：与可执行文件同目录的 `tuneux.toml`。
//!    这样把 `tuneux`（或 `tuneux.exe`）+ `tuneux.toml` 一起拷到 U 盘，
//!    单文件就是一个完整播放器，配置随身携带。
...
pub fn load() -> Config {
    ...
    let mut cfg = match load_from(&path) {
        Ok(cfg) => cfg,
        Err(e) => {
            // 不 panic，仅告知用户。首次运行文件不存在属正常情况，
            // 此处对"文件不存在"错误降级提示，避免误导。
            let note = if path.exists() {
                "配置解析失败，使用默认值"
            } else {
                "首次启动，使用默认配置"
            };
            eprintln!("[配置] {note}（{}）：{e}", path.display());
            Config::default()
        }
    };
