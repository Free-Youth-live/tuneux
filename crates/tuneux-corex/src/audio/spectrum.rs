...
//!   提供 2048 个有效 bin（实数 FFT 后半为镜像共轭）。低频段（30~200Hz）
//!   每 5 个 band 共用一个 bin，避免低频区多列同步。4096 点全 FFT（实数
//!   虚部置 0，等效 R2C 的前半）单次约 20μs，
...
//! ## 性能
//!
//! 4096-pt 全 FFT（实数虚部置 0，等效 R2C 前半） ≈ 20480 实数乘法（twiddle 优化后），单次 ~20μs 级。
